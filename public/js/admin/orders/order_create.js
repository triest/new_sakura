...
let js_vue_order_create = new Vue({
	...
	methods: {
		...
		load_next() {
			...
			// log('this.selected.type_service =>', this.selected.type_service, 'this.selected.select_date =>', this.selected.select_date);

			log(this.selected.services_date);

			if(this.selected.services_date.type_service && this.selected.services_date.select_date) {

				post('/admin/orders/ajax_load_block/' + this.selected.services_date.type_service.type, {
				...
				.then(data => {
					...
					scroll_to('.js-scroll-constructor_s1');
				})
				.catch(error => {
					console.log(error);
				})
